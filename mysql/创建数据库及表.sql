...
CREATE TABLE `airline`.`member_register_table` (
  `MEMBER_NO` INT UNSIGNED NOT NULL,		-- 会员卡号
  `TELEPHONE` CHAR(16) NOT NULL,			-- 会员手机号
  `PASSWORD` VARCHAR(16) NOT NULL,			-- 会员登录密码
  PRIMARY KEY (`MEMBER_NO`));

-- 3.创建管理员注册信息表 manager_register_table
CREATE TABLE `airline`.`manager_register_table` (
  `MANAGER_NO` INT UNSIGNED NOT NULL,		-- 管理员ID
  `TELEPHONE` CHAR(16) NOT NULL,			-- 管理员手机号
  `PASSWORD` VARCHAR(16) NOT NULL,			-- 管理员登录密码
...
CREATE TABLE `airline`.`member_information_table` (
  ...
  `IDENTITY_ID` INT UNSIGNED NOT NULL,		-- 身份证号
  `FFP_DATE` DATE NOT NULL,					-- 入会时间
  `FFP_TIER` INT UNSIGNED NOT NULL,			-- 会员卡级别
  `WORK_CITY` VARCHAR(4) NULL,				-- 工作地城市
  `WORK_PROVINCE` VARCHAR(4) NULL,			-- 工作地省份
  `WORK_COUNTRY` VARCHAR(4) NULL,			-- 工作地国家
...
CREATE TABLE `airline`.`flight_query_table` (
  ...
  `DEP_CT` VARCHAR(4) NOT NULL,				-- 起飞城市
  `DEP_CODE` VARCHAR(6) NOT NULL,			-- 起飞机场代号
  `ARR_CT` VARCHAR(4) NOT NULL,				-- 到达城市
  `ARR_CODE` VARCHAR(6) NOT NULL,			-- 到达机场代号
  `DEP_TERMINAL` VARCHAR(6) NOT NULL,		-- 起飞航站楼
  `ARR_TERMINAL` VARCHAR(6) NOT NULL,		-- 到达航站楼
  ...
  PRIMARY KEY (`FLIGHT_NO`, `DEP_CT`, `ARR_CT`, `DATE`));

-- 7.创建订单管理表 order_manage_table
CREATE TABLE `airline`.`order_manage_table` (
  `ORDER_NUMBER` INT UNSIGNED NOT NULL,		-- 订单号
  `FLIGHT_NO` VARCHAR(10) NOT NULL,			-- 航班号
  `DEP_CT` VARCHAR(4) NOT NULL,				-- 起飞城市
  `ARR_CT` VARCHAR(4) NOT NULL,				-- 到达城市
  `DATE` DATE NOT NULL,						-- 日期
  ...
  INDEX `Foreign_idx` (`FLIGHT_NO` ASC, `DEP_CT` ASC, `ARR_CT` ASC, `DATE` ASC) VISIBLE,
  CONSTRAINT `Foreign_2`
    FOREIGN KEY (`FLIGHT_NO` , `DEP_CT` , `ARR_CT` , `DATE`)
    REFERENCES `airline`.`flight_query_table` (`FLIGHT_NO` , `DEP_CT` , `ARR_CT` , `DATE`)
...
CREATE TABLE `airline`.`additional_ service_table` (
  `ORDER_NUMBER` INT UNSIGNED NOT NULL,		-- 订单号
  `BAGGAGE` VARCHAR(45) NULL,				-- 行李托运信息
  `INSURANCE` VARCHAR(45) NULL,				-- 保险信息
  `FOOD` VARCHAR(45) NULL,					-- 餐饮信息
  `CUSTOMER_SERVICE` VARCHAR(45) NULL,		-- 客户服务信息
  PRIMARY KEY (`ORDER_NUMBER`),
...
CREATE TABLE `airline`.`flight_information_table` (
  `ORDER_NUMBER` INT UNSIGNED NOT NULL,		-- 订单号
  `SEAT_NO` INT UNSIGNED NOT NULL,			-- 座位号
  `VALUE_ADD_SERVICE` VARCHAR(45) NULL,		-- 增值服务
  PRIMARY KEY (`ORDER_NUMBER`),
...
CREATE TABLE `airline`.`points_details_table` (
  `MEMBER_NO` INT UNSIGNED NOT NULL,		-- 用户会员卡号
...
CREATE TABLE `airline`.`mileage_upgrade_table` (
  ...
  `MILEAGE` INT UNSIGNED NOT NULL,			-- 升舱所需里程
  PRIMARY KEY (`FLIGHT_NO`));
